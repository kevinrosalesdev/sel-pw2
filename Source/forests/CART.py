...
class CART:
    ...
    def fit(self, training_set: pd.DataFrame, f = None):
        ...
        self.nodes[0] = self.Node(samples=self.ts, f=self.f, left=1, right=2)
    ...
    def _build_tree(self, split_left, split_right):
        ...
        if len(classes) > 1:
            self.nodes[len(self.nodes)] = self.Node(samples=split_left, f=self.f, left=len(self.nodes)+1, right=len(self.nodes)+2)
            left_set, right_set = self.nodes[len(self.nodes)-1].take_best_split()
            self._build_tree(left_set, right_set)
        else:
            self.nodes[len(self.nodes)] = self.Node(samples=split_left, f=self.f, result=classes[0])

        classes = pd.unique(split_right.iloc[:, -1])
        if len(classes) > 1:
            self.nodes[len(self.nodes)] = self.Node(samples=split_right, f=self.f, left=len(self.nodes)+1, right=len(self.nodes)+2)
            left_set, right_set = self.nodes[len(self.nodes)-1].take_best_split()
            self._build_tree(left_set, right_set)
        else:
            self.nodes[len(self.nodes)] = self.Node(samples=split_right, f=self.f, result=classes[0])
