...
def evaluate(dataset, train_test_ratio=0.8):
    ...
    rf = RandomForest(F=int(np.sqrt(train_set.shape[1]-1)), NT=10)
    rf.fit(train_set)
    end_time = datetime.now()
    print(f'Random Forest Training Time: {end_time - start_time}')
    start_time = datetime.now()
    prediction = rf.predict(test_set)
    end_time = datetime.now()
    print(f'Random Forest Classification Time: {end_time - start_time}')
